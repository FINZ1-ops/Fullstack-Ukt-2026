...
router.post("/", async (req, res) => {
  ...
  if (!customer_id) {
    return res.status(400).json({ status: "error", message: "customer_id wajib diisi" });
  }
  ...
  try {
    ...
    // Jika ada items, insert ke order_products & kurangi stok
    if (items && Array.isArray(items) && items.length > 0) {
      for (const item of items) {
        ...
        if (stockRes.rows.length === 0) {
          ...
          return res.status(400).json({ status: "error", message: `Produk ID ${product_id} tidak ditemukan` });
        }
        ...
        if (currentStock < quantity) {
          ...
          return res.status(400).json({
            status: "error",
            message: `Stok produk "${product.name}" tidak cukup. Stok tersedia: ${currentStock}`
          });
        }

        // Insert ke order_products (price opsional, tergantung schema)
        try {
          await client.query(
            `INSERT INTO order_products (order_id, product_id, quantity, price)
             VALUES ($1, $2, $3, $4)`,
            [order.id, product_id, quantity, price || 0]
          );
        } catch {
          // Fallback: kolom price tidak ada di schema
          await client.query(

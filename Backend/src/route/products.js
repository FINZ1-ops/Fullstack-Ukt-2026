...
router.post('/', verifyRole(["admin"]), async (req, res) => {
  ...
  const { name, price, size, color, category } = req.body;
...
router.delete('/:id', verifyRole(["admin"]), async (req, res) => {
  const client = await pool.connect();
  try {
    const check = await client.query('SELECT * FROM products WHERE id = $1', [req.params.id]);
    ...
    await client.query('DELETE FROM order_products WHERE product_id = $1', [req.params.id]);
    await client.query('DELETE FROM stocks WHERE product_id = $1', [req.params.id]);
    const { rows } = await client.query('DELETE FROM products WHERE id = $1 RETURNING *', [req.params.id]);
    await client.query('COMMIT');

    res.status(200).json({ status: "success", message: "Produk berhasil dihapus", data: rows[0] });
  } catch (err) {
    await client.query('ROLLBACK');
